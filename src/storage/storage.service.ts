...
export class StorageService {
  ...
  public async bucket(token: string, use: string, file: Express.Multer.File): Promise<string> {
    const storage = await this.storage(token, use, file);
    return storage.url;
  }


  async create(token: string, use: string, file: Express.Multer.File): Promise<{}> {
    const storage = await this.storage(token, use, file);
  ...
  async update(token: string, use: string, file: Express.Multer.File): Promise<{}> {
    const storage = await this.storage(token, use, file);
